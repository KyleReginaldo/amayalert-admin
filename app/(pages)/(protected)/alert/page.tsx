...
export default function AlertPage() {
  ...
  const handleCreate = async (alertData: AlertCreateRequest) => {
    try {
      ...
      const response = await alertsAPI.createAlert({ ...alertData, userId });
  ...
  const handleUpdate = async (id: number, alertData: AlertUpdate) => {
    try {
      ...
      const response = await alertsAPI.updateAlert(id, { ...alertData, userId });
      if (response.success && response.data) {
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Are you sure you want to delete this alert?')) {
      try {
        ...
        const response = await alertsAPI.deleteAlert(id, userId);
        if (response.success) {
          removeAlert(id);
        }
      } catch (error) {
          ...
          <div className="block divide-y md:hidden divide-border">
            {paginatedAlerts.length === 0 ? (
              ...
              paginatedAlerts.map((alert) => {
                const config = alertLevelConfig[alert.alert_level as keyof typeof alertLevelConfig];
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-3 mb-3">
                      <div className={`h-3 w-3 rounded-full mt-1.5 shrink-0 ${config?.dotColor}`} />
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mb-3">
                          <Badge variant="secondary" className={alertLevelColor(alert.alert_level)}>
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell>
                            <Badge
                              variant="secondary"
                              className={getAlertLevelColor(alert.alert_level)}
                            >
                              <LevelIcon className="w-3 h-3 mr-1" />
                              {config?.label}
                            </Badge>
