...
type MapProjection = { type: string; [key: string]: unknown } | string;
...
type MapProps = {
  ...
  /** Map projection type. Use `{ type: "globe" }` for 3D globe view when supported. */
...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  const { attributionControl: showAttribution = true, ...mapOptions } = props;
  ...
  useEffect(() => {
    ...
    const styleDataHandler = () => {
      ...
      styleTimeoutRef.current = setTimeout(() => {
        ...
        if (projection) {
          const mapWithProjection = map as MapWithProjection;
          mapWithProjection.setProjection?.(projection);
        }
      }, 100);
    };
